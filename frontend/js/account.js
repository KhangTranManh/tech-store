// Function declarations first, to avoid reference errors
...
function displayWishlistItems(wishlist, container) {
  ...
  if (!wishlist || !wishlist.items || !Array.isArray(wishlist.items) || wishlist.items.length === 0) {
    displayEmptyWishlist(container);
    return;
  }
  
  // Filter out any null items and sort by price (most expensive first)
  ...
  const sortedItems = [...validItems].sort((a, b) => b.price - a.price);
  ...
  items.forEach(item => {
    ...
    // If we still don't have a valid ID, generate a temporary one or use fallback
    if (!productId) {
      console.warn('No valid product ID found for item:', item.name);
      productId = item._id || ('temp-' + Date.now());
    }
    ...
    itemElement.innerHTML = `
      ...
      <div class="saved-item-info">
        <div class="saved-item-name">${item.name || 'Unknown Product'}</div>
        <div class="saved-item-price">${formattedPrice}</div>
        <div class="saved-item-actions">
          <button class="cart-btn" data-product-id="${productId}">Add to Cart</button>
...
function addToCart(productId, button) {
  ...
  if (window.cartFunctions && window.cartFunctions.addToCartLocal) {
    try {
      ...
      const itemContainer = button.closest('.saved-item');
      const productName = itemContainer.querySelector('.saved-item-name').textContent;
      const productPrice = parseFloat(itemContainer.querySelector('.saved-item-price').textContent.replace(/[^0-9.-]+/g,''));
...
function removeFromWishlist(productId) {
  // Check if we have a valid ID before sending the request
  if (!productId || productId === 'null' || productId === 'undefined') {
    console.error('Invalid product ID for removal:', productId);
    if (typeof window.showNotification === 'function') {
      window.showNotification('Cannot remove item: Invalid product ID', 'error');
    }
    return;
  }
...
// Now that all functions are defined, we can use them in the DOMContentLoaded event
